...
import com.loadium.postman2jmx.parser.XLMSorting;
...
public class Postman2Jmx {
	...
	public static void main(String[] args) {
		...
		try {
			...
			final String postmanCollectionJson = args[0];
			final String jmxOutputFile = args[1];

			logger.info("Trying to parse postman collection file: {}", postmanCollectionJson);
			final IParser parser = ParserFactory.getParser(CollectionVersion.V2);
			final PostmanCollection postmanCollection = parser.parse(postmanCollectionJson);
			...
			for (final PostmanItem postmanItem : postmanCollection.getItems()) {
				logger.debug("Name: {}", postmanItem.getName());
			}

			logger.info("Trying to build jmx file: {}", jmxOutputFile);
			final JmxFileBuilder jmxFileBuilder = new JmxFileBuilder();
			jmxFileBuilder.build(postmanCollection, jmxOutputFile);
			logger.info("Successfully build jmx file: {} ", jmxOutputFile);

			final XLMSorting xmlSorting = new XLMSorting();
			xmlSorting.sort(jmxOutputFile);
